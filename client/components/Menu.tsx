...
const Menu = () => {
	...
	return <div className='animate-fade-in opacity-0'>
		<div className='fixed hover:cursor-pointer transition-all right-0 bottom-0 shadow-xl hover:scale-110 m-10 bg-gradient rounded-full px-3 pt-2.5 p-2' onClick={() => setIsOpen(!isOpen)}>
			<Image src={MenuIcon} width={30} height={30} />
		</div>
		<div className={`${isOpen ? 'block' : 'hidden'} m-10 mb-24 mr-11 fixed right-0 bottom-0`}>
